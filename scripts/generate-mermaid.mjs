...
async function buildCISequence() {
  ...
  for (const f of yamls.slice(0, 4)) {
    ...
    if (jobsMatch) {
      ...
      for (const ln of afterJobs.split(/\r?\n/)) {
        // Match job names at 2-space indent
    ...
    for (const j of jobNames.slice(0, 6)) {
      ...
      // Extract job block: find the job name, then capture until next job or end of jobs section
      const jobLineRegex = new RegExp(`^  ${j}:\\s*$`, 'm');
      const jobMatch = jobLineRegex.exec(text);
      if (jobMatch) {
        const afterJob = text.slice(jobMatch.index + jobMatch[0].length);
        let jobBlock = '';
        for (const ln of afterJob.split(/\r?\n/)) {
          // Stop at next job (2-space indent followed by colon at end of line)
